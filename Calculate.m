function obj = Calculate(Data_info,Folder,Tif)
%% Keine Add-Ons notwendig! ff
% Ausgangsdatei
W1 = imread([Data_info(Folder,Tif).folder '\' Data_info(Folder,Tif)]); X1 = imresize(W1, 1);
% figure
% imshow(X1); title('1');
% figure
% imhist(X1)
...
obj.current_data = regionprops(A1,Z2,{'Centroid','Area','EquivDiameter','MinorAxisLength','MajorAxisLength','Centroid','WeightedCentroid'}); %data = regionprops(A1,Z1,{'Centroid','WeightedCentroid'});
% figure
...
numObj = numel(obj.current_data);
hold on
plot(x,y,'ro');
for n = 1 : numObj
    %plot(obj.current_data(n).WeightedCentroid(1), obj.current_data(n).WeightedCentroid(2), 'ro')
    % Abstand zwischen Nozzle und Flächenschwerpunkt berrechnen und an struct übergeben
    obj.current_data(n).AbsoluteValue=sqrt(((obj.current_data(n).Centroid(1)-x)^2)+(obj.current_data(n).Centroid(2)-y)^2);
    %plot(obj.current_data(n).Centroid(1), obj.current_data(n).Centroid(2), 'b*')
end
hold off
